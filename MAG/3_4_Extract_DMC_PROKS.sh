...
OUTPUTDir="/gxfs_work/geomar/smomw681/DATA/MAG_Illumina/DEEPMicroClass/CLASS_CONTIGs/PROKS"
...
if [ ! -f ${OUTPUTDir}/${i}_contigs_min500_Proks.fasta ]; then
    echo working with $i
    base=$(basename $i "_contigs_min500_renamed.fasta");
    DeepMicroClass extract \
        --fasta $i \
        --output ${OUTPUTDir}/${base}_contigs_min500_Proks.fasta \
        --class Prokaryote \
