...
ORF_ARG_FILEs=(${DeepARGsDIR}/*.deeparg.out.mapping.ARG)

# Original DeepARG summary script 
## count number of hits in ORF files
wc -l ${DeepARGsDIR}/*.deeparg.out.mapping.ARG | awk '$1 > 1 {print $1,$2}'
## the total hits
awk 'NR > 1' ${DeepARGsDIR}/*.deeparg.out.mapping.ARG | wc -l | awk '$1 > 1 {print $1,$2}'
## showing for each file, excluding the header
wc -l ${DeepARGsDIR}/*.deeparg.out.mapping.ARG | awk '$1 > 1 {print $1-1"\t"$2}' > /gxfs_work/geomar/smomw681/DATA/GENOME_Jutta/ARG_Analysis_strains/DeepARGs_ORF_hits_perSample.txt

## Summary using perl script :
INPUT_FILE="/gxfs_work/geomar/smomw681/DATA/GENOME_Jutta/ARG_Analysis_strains/DeepARGs_ORF_hits_perSample.txt"  
OUTPUT_FILE="/gxfs_work/geomar/smomw681/DATA/GENOME_Jutta/ARG_Analysis_strains/deeparg_ALL_summary.txt"
perl /gxfs_work/geomar/smomw681/DATA/MAG_Files/SL_Summarize_deeparg_out_mapping_ARG_with_InOutOptions.pl "$INPUT_FILE" "$OUTPUT_FILE"
##
