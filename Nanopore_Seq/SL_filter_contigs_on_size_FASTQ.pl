...
($fileBaseName, $dirName, $fileExtension) = fileparse($input_file, qr/\.(fa|fasta|fastq|fna|faa)$/ );
...
if (not defined $filePrefix) {
  ...
  chomp $filePrefix;
  open (OUTFILE, "> $filePrefix$outputfile") || die "Can't open output file.\n";
  }

if $fileExtension eq ".fastq" or $fileExtension eq ".fq" {
  my $counter=0;
  my $header = <$IN>;
  my $seq = "";
  my $plus = <$IN>;
...
} elsif $fileExtension eq ".fasta" or $fileExtension eq ".fa" {
  my $counter=0;
  my @line;
  my $seq = "";
  my $name = "";
...
while (<INFILE>) {
  ...
  if ($_ =~ /^[@]/) {
    $name = $_;
    $counter++;
    if($seq ne "" && $header ne "" && (length($seq) >= $min_contig_size)){
