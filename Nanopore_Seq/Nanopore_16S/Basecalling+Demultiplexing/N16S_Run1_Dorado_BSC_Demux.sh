...
#SBATCH --output=N16S_Run1_Dorado_1.out
#SBATCH --error=N16S_Run1_Dorado_1.err
...
DEMUX_DIR="/gxfs_work/geomar/smomw681/NANOPORE_DATA/DEMULTIPLEXED/NANOPORE_16S_Run1_DEMUX"
...
dorado basecaller $MODEL_DIR/ $RAW_DIR/ --kit-name SQK-16S114-24 --no-trim --device cpu --output-dir $BASECALL_DIR/ > NANOPORE_16S_Run1_basecalled.bam
echo done with basecalling

echo Start Demultiplexing
dorado demux $BASECALL_DIR/*.bam --kit-name SQK-16S114-24 --output-dir $DEMUX_DIR/ --emit-fastq --threads 16
